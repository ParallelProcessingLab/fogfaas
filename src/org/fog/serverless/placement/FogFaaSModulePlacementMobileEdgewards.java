...
public class FogFaaSModulePlacementMobileEdgewards extends FogFaaSModulePlacement{
	
	protected FogFaaSModuleMapping moduleMapping;
	protected List<FogFaaSSensor> sensors;
	protected List<FogFaaSActuator> actuators;
	...
	public FogFaaSModulePlacementMobileEdgewards(List<FogFaaSDevice> fogDevices, List<FogFaaSSensor> sensors, List<FogFaaSActuator> actuators, 
			FogFaaSApplication application, FogFaaSModuleMapping moduleMapping){
	...
	private List<String> getModulesToPlace(List<String> placedModules){
		...
		for(String moduleName : modulesToPlace_1){
			...
			for(FogFaaSAppEdge edge : app.getEdges()){
				//CHECK IF OUTGOING DOWN EDGES ARE PLACED
				if(edge.getSource().equals(moduleName) && edge.getDirection()==FogFaaSTuple.DOWN && !placedModules.contains(edge.getDestination()))
					toBePlaced = false;
				//CHECK IF INCOMING UP EDGES ARE PLACED
				if(edge.getDestination().equals(moduleName) && edge.getDirection()==FogFaaSTuple.UP && !placedModules.contains(edge.getSource()))
					toBePlaced = false;
			}
			if(toBePlaced)
	...
	protected double getRateOfSensor(String sensorType){
		for(FogFaaSSensor sensor : getSensors()){
	...
	private void placeModulesInPath(List<Integer> path) {
		...
		Map<FogFaaSAppEdge, Double> appEdgeToRate = new HashMap<FogFaaSAppEdge, Double>();
		
		/**
		 * Periodic edges have a fixed periodicity of tuples, so setting the tuple rate beforehand
		 */
		for(FogFaaSAppEdge edge : getApplication().getEdges()){
			if(edge.isPeriodic()){
		...
		for(Integer deviceId : path){
			FogFaaSDevice device = getFogDeviceById(deviceId);
			...
			while(changed){		//Loop runs as long as some new information is added
				changed=false;
				Map<FogFaaSAppEdge, Double> rateMap = new HashMap<FogFaaSAppEdge, Double>(appEdgeToRate);
				for(FogFaaSAppEdge edge : rateMap.keySet()){
					FogFaaSAppModule destModule = getApplication().getModuleByName(edge.getDestination());
					...
					for(Pair<String, String> pair : map.keySet()){
						if(pair.getFirst().equals(edge.getTupleType())){
							...
							FogFaaSAppEdge outputEdge = getApplication().getEdgeMap().get(pair.getSecond());
			...
			while(modulesToPlace.size() > 0){ // Loop runs until all modules in modulesToPlace are deployed in the path
				...
				if(upsteamDeviceId > 0){
					if(upsteamDeviceId==deviceId){
						...
						for(FogFaaSAppEdge edge : getApplication().getEdges()){		// take all incoming edges
	...
	private List<String> shiftModuleNorth(String moduleName, double cpuLoad, Integer deviceId, List<String> operatorsToPlace) {
		...
		while(true){ // Loop iterates over all devices in path upstream from current device. Tries to place modules (to be shifted northwards) on each of them.
			...
			FogFaaSDevice fogDevice = getFogDeviceById(id);
	...
	private List<String> findModulesToShift(List<String> modules, Integer deviceId) {
		...
		while(changed){ // Keep loop running as long as new information is added.
			 ...
			 */
			for(FogFaaSAppEdge edge : getApplication().getEdges()){
				if(upstreamModules.contains(edge.getSource()) && edge.getDirection()==FogFaaSTuple.UP && 
						getCurrentModuleMap().get(deviceId).contains(edge.getDestination()) 
						&& !upstreamModules.contains(edge.getDestination())){
					upstreamModules.add(edge.getDestination());
					changed = true;
	 ...
	 */
	private Map<String, Integer> getAssociatedSensors(FogFaaSDevice device) {
	 ...
	 */
	private Map<String, Integer> getAssociatedActuators(FogFaaSDevice device) {
		Map<String, Integer> endpoints = new HashMap<String, Integer>();
	...
	protected List<List<Integer>> getPaths(final int fogDeviceId){
		FogFaaSDevice device = (FogFaaSDevice)CloudSim.getEntity(fogDeviceId); 
