...
/**
 * Implementation class for simulating an intelligent video camera system within
 * Data Center Networks (DCNs) using the FogFaaS environment.
 * 
 * This class provides a simulation of a video camera system designed for
 * intelligent monitoring and analysis within a data center context. It
 * leverages FogFaaS to model resource allocation, network communication, and
 * processing of video streams across fog and cloud resources. This example
 * highlights the use of FogFaaS for performance evaluation of video analytics
 * applications in DCNs. The simulation is configured to use a default of
 * `numOfAreas = 3` and `numOfCamerasPerArea = 5`, representing the number of
 * monitoring areas and cameras per area, respectively. These parameters can be
 * adjusted to explore different DCN configurations.
 * 
 * @author Ghaseminya
 */
...
public class DCNS_FogFaaS {
	...
	public static void main(String[] args) {
		Log.printLine("Starting DCNS FogFaaS version...");
		try {
	...
	private static FogFaaSApplication createApplication(String appId, int userId) {
		 ...
		 */
		application.addAppEdge("CAMERA", "motion_detector", 1000, 20000, "CAMERA", FogFaaSTuple.UP,
				FogFaaSAppEdge.SENSOR); // adding edge from CAMERA (sensor) to Motion Detector module carrying tuples of
										// type CAMERA
		application.addAppEdge("motion_detector", "object_detector", 2000, 2000, "MOTION_VIDEO_STREAM", FogFaaSTuple.UP,
				FogFaaSAppEdge.MODULE); // adding edge from Motion Detector to Object Detector module carrying tuples of
										// type MOTION_VIDEO_STREAM
		application.addAppEdge("object_detector", "user_interface", 500, 2000, "DETECTED_OBJECT", FogFaaSTuple.UP,
				FogFaaSAppEdge.MODULE); // adding edge from Object Detector to User Interface module carrying tuples of
										// type DETECTED_OBJECT
		application.addAppEdge("object_detector", "object_tracker", 1000, 100, "OBJECT_LOCATION", FogFaaSTuple.UP,
				FogFaaSAppEdge.MODULE); // adding edge from Object Detector to Object Tracker module carrying tuples of
										// type OBJECT_LOCATION
		application.addAppEdge("object_tracker", "PTZ_CONTROL", 100, 28, 100, "PTZ_PARAMS", FogFaaSTuple.DOWN,
				FogFaaSAppEdge.ACTUATOR); // adding edge from Object Tracker to PTZ CONTROL (actuator) carrying tuples
											// of type PTZ_PARAMS
		...
		final FogFaaSAppLoop loop2 = new FogFaaSAppLoop(new ArrayList<String>() {
			{
				add("object_tracker");
				add("PTZ_CONTROL");
			}
	...
	private static FogFaaSDevice addArea(String id, int userId, String appId, int parentId) {
		...
		for (int i = 0; i < numOfCamerasPerArea; i++) {
			...
			FogFaaSDevice camera = addCamera(mobileId, userId, appId, router.getId()); // adding a smart camera to the
																						// physical topology. Smart
																						// cameras have been modeled as
																						// fog devices as well.
	...
	private static FogFaaSDevice addCamera(String id, int userId, String appId, int parentId) {
		...
		FogFaaSSensor sensor = new FogFaaSSensor("s-" + id, "CAMERA", userId, appId, new DeterministicDistribution(5)); // inter-transmission
																														// time
																														// of
																														// camera
																														// (sensor)
	...
	private static ArrayList<FogFaaSInvoker> createFogList(int brokerId) throws Exception {
		...
		for (int i = 0; i < Constants.NUMBER_FOG; ++i) {
			...
			for (int j = 0; j < Constants.FOG_PES[vmType]; ++j) {
				// ContainerVmBwProvisionerSimple provisioner = new
				// ContainerVmBwProvisionerSimple(11);
			...
			PowerHost host = new PowerHost(hostId, new RamProvisionerSimple((int) Constants.FOG_RAM[vmType]),
					new BwProvisionerOverbooking(bw), storage, pes, new StreamOperatorScheduler(pes),
			...
			containerFoGs.add(new FogFaaSInvoker(IDs.pollId(ContainerVm.class), brokerId,
					(double) Constants.FOG_MIPS[vmType], (float) Constants.FOG_RAM[vmType], Constants.FOG_BW,
					Constants.FOG_SIZE, "Xen", new FogFaaSContainerScheduler(peList),
					new FogFaaSContainerRamProvisioner(Constants.FOG_RAM[vmType]),
					new ContainerBwProvisionerSimple(Constants.FOG_BW), peList, Constants.SCHEDULING_INTERVAL,
	...
	private static void printContainerList(List<FogFaaSContainer> list) {
		...
		Log.printLine("Container ID" + indent + "VM ID" + indent + "Start Time" + indent + "Finish Time" + indent
				+ "Finished Requests List");
		DecimalFormat dft = new DecimalFormat("###.##");
		for (int i = 0; i < size; i++) {
			container = list.get(i);
			Log.print(indent + container.getId() + indent + indent);
			Log.printLine(
					indent + (container.getVm()).getId() + indent + indent + (dft.format(container.getStartTime()))
							+ indent + indent + indent + indent + dft.format(container.getFinishTime()) + indent
							+ indent + indent + indent + container.getfinishedTasks());
		}
	...
	private static void printRequestList(List<ContainerCloudlet> list) {
		...
		Log.printLine("request ID" + indent + "Function ID" + indent + "Container ID" + indent + "STATUS" + indent
				+ "Data center ID" + indent + "Final VM ID" + indent + "Execution Time" + indent + "Start Time" + indent
				+ "Finish Time" + indent + "Response Time" + indent + "Vm List");
		DecimalFormat dft = new DecimalFormat("###.##");
		for (int i = 0; i < size; i++) {
			request = list.get(i);
			...
			if (request.getCloudletStatusString() == "Success") {
				...
				Log.print("SUCCESS");
				if (Math.ceil((request.getFinishTime() - ((FogFaaSRequest) request).getArrivalTime())) <= (Math
						.ceil(((FogFaaSRequest) request).getMaxExecTime()))
						|| Math.floor((request.getFinishTime() - ((FogFaaSRequest) request).getArrivalTime())) <= (Math
								.ceil(((FogFaaSRequest) request).getMaxExecTime()))) {
					deadlineMetStat++;
				}
			} else {
			...
			Log.printLine(indent + indent + request.getResourceId() + indent + indent + indent + indent
					+ request.getVmId() + indent + indent + indent + indent + dft.format(request.getActualCPUTime())
					+ indent + indent + indent + dft.format(request.getExecStartTime()) + indent + indent + indent
					+ dft.format(request.getFinishTime()) + indent + indent + indent
					+ dft.format(request.getFinishTime() - ((FogFaaSRequest) request).getArrivalTime()) + indent
					+ indent + indent + ((FogFaaSRequest) request).getResList());
		}
		Log.printLine("Deadline met no: " + deadlineMetStat);
	}
}
