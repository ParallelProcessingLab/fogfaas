package org.fog.serverless.application;
 ...
 */
public class FogFaaSApplication {
	 ...
	 */
	private List<FogFaaSAppModule> modules;
	
	/**
	 * List of application edges in the application
	 */
	private List<FogFaaSAppEdge> edges;
	
	/**
	 * List of application loops to monitor for delay
	 */
	private List<FogFaaSAppLoop> loops;
	
	private Map<String, FogFaaSAppEdge> edgeMap;
	 ...
	 * @return
	 */
	public static FogFaaSApplication createApplication(String appId, int userId){
		return new FogFaaSApplication(appId, userId);
	}
	...
	public void addAppModule(String moduleName, int ram){
		...
		FogFaaSAppModule module = new FogFaaSAppModule(FogUtils.generateEntityId(), moduleName, appId, userId, 
				mips, ram, bw, size, vmm, new TupleScheduler(mips, 1), new HashMap<Pair<String, String>, SelectivityModel>(),true);
	...
	public void addAppModule(String moduleName, int ram, int mips, int size) {
		...
		FogFaaSAppModule module = new FogFaaSAppModule(FogUtils.generateEntityId(), moduleName, getAppId(), getUserId(),
				mips, ram, bw, size, vmm, new TupleScheduler(mips, 1), new HashMap<Pair<String, String>, SelectivityModel>(),true);
	...
	public void addAppEdge(String source, String destination, double tupleCpuLength, 
		...
		FogFaaSAppEdge edge = new FogFaaSAppEdge(source, destination, tupleCpuLength, tupleNwLength, tupleType, direction, edgeType);
		getEdges().add(edge);
	...
	public void addAppEdge(String source, String destination, double periodicity, double tupleCpuLength, 
			double tupleNwLength, String tupleType, int direction, int edgeType){
		FogFaaSAppEdge edge = new FogFaaSAppEdge(source, destination, periodicity, tupleCpuLength, tupleNwLength, tupleType, direction, edgeType);
		getEdges().add(edge);
		getEdgeMap().put(edge.getTupleType(), edge);
	}
	 ...
	 * @return
	 */
	public List<FogFaaSAppEdge> getPeriodicEdges(String srcModule){
		List<FogFaaSAppEdge> result = new ArrayList<FogFaaSAppEdge>();
		for(FogFaaSAppEdge edge : edges){
	...
	public FogFaaSApplication(String appId, int userId) {
		setAppId(appId);
		setUserId(userId);
		setModules(new ArrayList<FogFaaSAppModule>());
		setEdges(new ArrayList<FogFaaSAppEdge>());
		setGeoCoverage(null);
		setLoops(new ArrayList<FogFaaSAppLoop>());
		setEdgeMap(new HashMap<String, FogFaaSAppEdge>());
	}
	
	public FogFaaSApplication(String appId, List<FogFaaSAppModule> modules,
			List<FogFaaSAppEdge> edges, List<FogFaaSAppLoop> loops, GeoCoverage geoCoverage) {
		setAppId(appId);
		setModules(modules);
		setEdges(edges);
	 ...
	 * @return
	 */
	public FogFaaSAppModule getModuleByName(String name){
		for(FogFaaSAppModule module : modules){
	 ...
	 * @return
	 */
	public List<FogFaaSTuple> getResultantTuples(String moduleName, FogFaaSTuple inputTuple, int sourceDeviceId, int sourceModuleId){
		List<FogFaaSTuple> tuples = new ArrayList<FogFaaSTuple>();
		FogFaaSAppModule module = getModuleByName(moduleName);
		for(FogFaaSAppEdge edge : getEdges()){
			if(edge.getSource().equals(moduleName)){
				...
				if(selectivityModel.canSelect()){
					...
					if(edge.getEdgeType() == FogFaaSAppEdge.ACTUATOR){
						...
						FogFaaSTuple tuple = new FogFaaSTuple(appId, FogUtils.generateTupleId(), edge.getDirection(),  
							...
							tuple.setDirection(FogFaaSTuple.ACTUATOR);
	 ...
	 * @return
	 */
	public FogFaaSTuple createTuple(FogFaaSAppEdge edge, int sourceDeviceId, int sourceModuleId){
		FogFaaSAppModule module = getModuleByName(edge.getSource());
		if(edge.getEdgeType() == FogFaaSAppEdge.ACTUATOR){
			for(Integer actuatorId : module.getActuatorSubscriptions().get(edge.getTupleType())){
				FogFaaSTuple tuple = new FogFaaSTuple(appId, FogUtils.generateTupleId(), edge.getDirection(),  
						(long) (edge.getTupleCpuLength()),
	...
	public List<FogFaaSAppModule> getModules() {
		return modules;
	}
	public void setModules(List<FogFaaSAppModule> modules) {
		this.modules = modules;
	}
	public List<FogFaaSAppEdge> getEdges() {
		return edges;
	}
	public void setEdges(List<FogFaaSAppEdge> edges) {
		this.edges = edges;
	}
	...
	public List<FogFaaSAppLoop> getLoops() {
		return loops;
	}

	public void setLoops(List<FogFaaSAppLoop> loops) {
		this.loops = loops;
	}
	...
	public Map<String, FogFaaSAppEdge> getEdgeMap() {
		return edgeMap;
	}

	public void setEdgeMap(Map<String, FogFaaSAppEdge> edgeMap) {
		this.edgeMap = edgeMap;
	}

	public List<String> getModuleNames(){
		List<String> appModuleNames = new ArrayList<>();
		for(FogFaaSAppModule module:getModules()){
	...
	public void createDAG() {
		...
		for (FogFaaSAppEdge edge : getEdges()) {
			if (edge.getDirection() == FogFaaSTuple.UP)
