...
public class FogFaaSSensor extends SimEntity{
	...
	public FogFaaSSensor(String name, int userId, String appId, int gatewayDeviceId, double latency, GeoLocation geoLocation, 
	 ...
	 */
	public FogFaaSSensor(String name, String tupleType, int userId, String appId, Distribution transmitDistribution) {
		super(name);
		this.setAppId(appId);
		this.setTransmitDistribution(transmitDistribution);
		setTupleType(tupleType);
	...
	public void transmit(){
		FogFaaSAppEdge _edge = null;
		for(FogFaaSAppEdge edge : getApp().getEdges()){
		...
		FogFaaSTuple tuple = new FogFaaSTuple(getAppId(), FogUtils.generateTupleId(), FogFaaSTuple.UP, cpuLength, 1, nwLength, outputSize, 
	...
	protected int updateTimings(String src, String dest){
		FogFaaSApplication application = getApp();
		for(FogFaaSAppLoop loop : application.getLoops()){
